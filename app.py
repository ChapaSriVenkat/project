...
def login(username, password):
    ...
    if user and password == user[3]:  # direct comparison with stored raw password
        # Generate new JWT
        token = jwt.encode(
            {"username": username, "exp": datetime.datetime.utcnow() + datetime.timedelta(hours=1)},
            SECRET_KEY,
...
if st.session_state["page"] == "Signup":
    ...
    new_pass = st.text_input("Password (will be stored as plain text)", type="password", key="signup_pass")
